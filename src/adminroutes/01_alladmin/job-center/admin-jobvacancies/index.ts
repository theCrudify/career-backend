...
/**
 * Mendapatkan daftar lowongan pekerjaan dengan filter admin
 * Termasuk yang berstatus draft dan yang tidak aktif
 */
export const get = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const statusRequisition = req.query.status_requisition as string;
    ...
    // Sorting
    const sort_by = req.query.sort_by as string || 'jr.created_at'; 
    ...
    // Filter berdasarkan status aktif/nonaktif (untuk admin, kita bisa menampilkan semua jika tidak ada filter)
    if (isActive) {
    ...
    // Filter berdasarkan status requisition (draft, submitted, approved, dll)
    if (statusRequisition) {
      query += ` AND jr.status_requisition = ?`;
      params.push(statusRequisition);
    }
    ...
    if (statusRequisition) countQuery += ` AND jr.status_requisition = ?`;
    ...
    const statusRequisitions = await db.$queryRaw`
      SELECT DISTINCT status_requisition FROM tr_job_requisition 
      WHERE status_requisition IS NOT NULL
      ORDER BY status_requisition
    `;

    const statusOptions = await db.$queryRaw`
      SELECT DISTINCT status FROM tr_job_requisition 
      WHERE status IS NOT NULL
      ORDER BY status
    `;

    // Ambil departemen untuk filter
    ...
    // Ambil education levels untuk filter
    ...
    return res.status(200).json({ 
      ...
      filters: {
        ...
        sites,
        educations,
        statusRequisitions: convertBigIntToString(statusRequisitions),
        statusOptions: convertBigIntToString(statusOptions)
      },
      pagination: {
        currentPage: page,
